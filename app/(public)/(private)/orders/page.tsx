...
const OrdersPage = () => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (!debouncedSearch) return data.data.orders;

    const query = debouncedSearch.toLowerCase();
    return data.data.orders.filter((order) =>
  ...
  }, [data?.data.orders, debouncedSearch]);
  ...
  const showLoadMore = useMemo(() => {
    if (!data?.data.pagination) return false;
    return page < data.data.pagination.totalPages;
